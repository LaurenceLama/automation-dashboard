    ...
    <main className="min-h-screen p-6 flex items-center justify-center">
      {!createdWorkflow ? (
        ...
        <section className="space-y-60 max-sm:max-w-xs">
          ...
          <div className="space-y-6">
            <p className="text-blue-400 font-semibold text-3xl">
              Step 4 — Add Error Handlers to show Failed Executions
            </p>

            <p className="text-sm sm:text-base">
              Set your Error Handlers also to POST. Then choose Raw JSON again
              to paste this payload:
            </p>

            <pre className="overflow-x-auto text-xs sm:text-sm">
              {`{             REQUIRED PAYLOAD:
                "workflowKey": "${createdWorkflow.webhook_path}",
                "error_message": "{{error.message}}",     <- Use your platform's error variable
                "status": "error"
                    
              OPTIONAL PAYLOAD:
                "error_type": "{{error.type}}",           <- Use your platform's error type variable
                "name": "client name",
                "email": "client email"      <- don't forget that the last payload should NOT have a comma!
              }`}
            </pre>
            ...
            <pre className="overflow-x-auto text-xs sm:text-sm">
              ...
              workflowKey       ${createdWorkflow.webhook_path}
              error_message     Use your platform's error variable (e.g. {{error.message}} on Make, Zapier uses error output)
              status            error

              error_type        Use your platform's error variable (e.g. {{error.type}} on Make)
              email             client email
              name              client name
              `}
            </pre>
            ...
            <p className="text-sm sm:text-base">
              Note that the error variables contain a number that indicates what
              specific action located in your workflow.
              <br />
              Usually looks like{" "}
              <p className="inline p-1 bg-blue-600 rounded-sm font-bold text-sm">
                3. Error: Message
              </p>{" "}
              (on Make) or{" "}
              <p className="inline p-1 bg-white text-black rounded-sm font-bold text-sm">
                (step image) 2. Error Message
              </p>{" "}
              (on Zapier)
            </p>

            <p className="text-sm sm:text-base font-semibold text-green-600">
              ✅ With error handlers, you&apos;ll capture both successful and
              failed executions.
            </p>
          </div>
          ...
          <div className="space-y-4 max-sm:space-y-10">
            <div className="space-y-1">
              ...
              <p className="text-sm sm:text-md opacity-80">
                This step only reports <b>successful</b> workflow runs. If your
                automation fails before this step, the execution will not appear
                in the dashboard.
              </p>
              <p className="text-sm sm:text-base opacity-80">
                To see failures, set up error handlers (Step 4 above).
              </p>
            </div>
